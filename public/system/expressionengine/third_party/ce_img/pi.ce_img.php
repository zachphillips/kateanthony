...
====================================================================================================
 ...
 copyright 		Copyright (c) 2011 Causing Effect, Aaron Waldon <aaron@causingeffect.com>
...
class Ce_img 
{
	...
	public function get_make_defaults()
	{
		...
		$defaults['disable_xss_check'] = ( strtolower( $this->EE->TMPL->fetch_param('disable_xss_check') ) == "yes" ) ? TRUE : FALSE;
		...
		//max param
		$defaults['max'] = $this->EE->TMPL->fetch_param('max');

		//height param
		$height = $this->EE->TMPL->fetch_param('height', 0);
		if ( $height == 0 ) //check to see if the user used 'max_height' instead of 'height'
		{
			$height = $this->EE->TMPL->fetch_param('max_height', 0);
		}
		$defaults['height'] = $height;

		//width param
		$width = $this->EE->TMPL->fetch_param('width', 0);
		if ( $width == 0 ) //check to see if the user used 'max_width' instead of 'width'
		{
			$width = $this->EE->TMPL->fetch_param('max_width', 0);
		}
	...
	public function make()
	{
		...
		if ( $src == '' && $defaults['fallback_src'] == '' )
		{
			if ( $debug )
			{
				$this->EE->TMPL->log_item( 'Source and fallback source cannot both be blank.' );
			}
